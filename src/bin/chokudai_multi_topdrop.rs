...
fn solve2(input: &Input, step: i32, xnum: usize, snum: usize, steps: usize) -> i32 {
    ...
    loop {
        ...
        range.into_par_iter().for_each(|i| {
            ...
            let result = solve3(ii, start_id, d, xnum, snum, step, drop, &next);
...
fn solve3(
    ...
    let mut dropflag = vec![false; perm_length];
    let mut takeflag = vec![false; perm_length];
    ...
    while now_size * (xnum - 1) + aftersnum * step < 1000000 {
        ...
        for i in array_start + 1..base_array.len() {
            if base_array[i] < 4 {
                ...
                if takeflag[i] {
                    let max = now_array.len().min(takenum);
                    for k in 0..max {
                        next_array.push(now_array[k]);
                    }
                } else if dropflag[i] {
                    if now_array.len() > takenum {
                        for k in takenum..now_array.len() {
                            next_array.push(now_array[k]);
                        }
                    }
                } else {
    ...
    for turn in 0..now_size {
        ...
        if !visited[now] {
            ...
            visited[now] = true;
            if remain_pos == 0 {
                println!("OK {} {} take:{}", remain_pos, turn, takenum);
